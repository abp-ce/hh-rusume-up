...
func refreshToken(envVars *EnvVars, logger *log.Logger) {
	data := map[string]string{"grant_type": "refresh_token", "refresh_token": envVars.refreshToken}
	jsonData, err := json.Marshal(data)
	if err != nil {
		logger.Println("Error encoding refresh JSON:", err)
		return
	}
	resp, err := http.Post(hhUrl + "/oauth/token", "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
	...
	envVars.accessToken = result["refresh_token"].(string)
